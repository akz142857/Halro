...
- **审计乱序/序号断裂检测只有 E1**：代码在 `internal/audit/log.go:341-347`，但 8 个测试函数没有
  一个构造出带缺口的帧，篡改测试先被 HMAC 拦住（260918-PV-F-18）。
- **没有任何审计完整性告警**（只有针对锚点发射的 `HalroAuditAnchorStale`），也没有证明「管理面
  变更都被审计」的 sweep（260918-PV-F-19）。
...
| 260918-PV-F-18 | P2 | 审计乱序/序号断裂检测只有代码没有测试，篡改测试先被 HMAC 拦截 | OPEN |
| 260918-PV-F-19 | P2 | 没有审计完整性告警，也没有「管理面变更都被审计」的穷举证明 | OPEN |
...
3. 补 260918-PV-F-18、260918-PV-F-19 的自动化覆盖（**F-21 已于 2026-09-24 处置**，剩余部分需目标环境）；给 260918-PV-F-07 定阈值后补告警规则。
