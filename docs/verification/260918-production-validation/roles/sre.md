...
2. ~~**The plan asks G5 to record 流式首字节 p95/p99, and Halro has no first-byte metric at all.**~~ **Resolved 2026-09-24** by adding the metric: `halro_stream_first_byte_seconds{operation,le}`, measured from request arrival to the first streamed event written *and flushed*, split across the four northbound streaming faces, on the same 15 bucket bounds as every other latency series here. The load-harness option was rejected: it would have meant the number watched in production and the number signed came from two different instruments. What remains for §5 row 2 is the **threshold**, not the measurability.
