...
| [Provider Offering 与订阅接入统一方案](prd/provider-offering-subscription-access-plan.zh-CN.md) | 阶段 1 已实施：Offering 与产品地域声明在 Access Surface 层（凭据的解析键），Profile 派生；`/admin/api/v1/provider-profiles` 下发 offerings/region_scope/region_hosts，控制台的凭据与连接表单改为元数据驱动，`type === "bedrock"` 分支清除。同时修掉并暴露了一处存量缺陷：凭据创建不发 access_surface，海外 BigModel 凭据一律落在 CN surface 上——写路径改为有歧义即拒绝，`halro doctor` 新增 `credential_product` 检查（该文 §8.3、§14）。阶段 0/2/3/4（Coding Plan、Token Plan、OAuth）需要真实订阅账号证据，未实施 |
