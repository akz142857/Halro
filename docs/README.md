...
| [生产验证执行记录 · 2026-09-25 · G0 重新冻结](verification/production-validation-run-260925-g0.zh-CN.md) | 把 260918-PV-F-01 关掉：9 个 PR 全部合入后用干净提交 `9e0d73c7` 重跑 G0——完整门禁与 CI 在同一 SHA 上通过，版本串不再带 `-dirty`，制品 digest 在固定 `SOURCE_DATE_EPOCH` 下可复现。重跑本身发现 `make build` 会留下一个自称来自不干净树的旧 `halro-deadman`（260925-PV-F-01，已修并加门禁）。G0 因缺发布制品与签名停在 `CONDITIONAL PASS`，整体仍 `NO-GO` |
...
| [Claude 订阅凭据实测证据 · 2026-09-22](verification/anthropic-claude-subscription-evidence.md) | #351 的实测一半：用操作者自己的 Claude Code 订阅打真实 `/v1/messages`。`x-api-key` 送订阅 token 得 `401 API key is invalid.`——正是 Halro 的 Anthropic profile 会送出的形状，且这句话把操作者引向查错别字；同一把 token 以 `Authorization: Bearer` 送出则得 200，所以**边界是条款，不是上游在拦**。结论：拒绝必须是 Halro 自己的 fail-closed 决定，不能等上游说不 |
| [Codex 订阅凭据实测证据 · 2026-09-23](verification/codex-subscription-evidence.md) | #350 Gate 0 的实测一半：带订阅 Bearer token、身份头照实写 `Halro/0.8.5` + `originator: halro`、不借用任何 Codex 标识，`chatgpt.com/backend-api/codex/responses` 返回 **400** 且拒的是 model 不是身份——**认证与客户端身份都被接受**。据此推翻「必须冒充 Codex CLI」的二手说法，(c) 不构成阻塞；拦住这件事的只剩条款 (b)，而上游不执法 |
