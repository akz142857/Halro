...
| KMS extension boundary | accepted single-module/single-artifact AWS SDK layout backed by reproducible module/binary/container/build/test/cold-start/SBOM/vulnerability measurements; provider-neutral bounded `kms.Wrapper`; official AWS KMS Adapter with directly provable Workload Identity sources, symmetric Encrypt/Decrypt, strict Key/account/region/endpoint/algorithm allowlists, versioned Encryption Context, canonical protected payload, typed errors and core-owned bounded retry; unknown errors fail closed; authenticated fake KMS contract, File-mode live metadata zero-call proof, and CI-enforced Gateway/core dependency isolation; real AWS evidence remains pending |
