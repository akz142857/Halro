...
| KMS extension boundary | accepted single-module/single-artifact AWS SDK layout backed by reproducible module/binary/container/build/test/cold-start/SBOM/vulnerability measurements; provider-neutral bounded `kms.Wrapper`, stable typed error taxonomy, authenticated fake KMS timeout/cancellation/fault contract, File-mode live metadata zero-call proof, and CI-enforced Gateway/core dependency isolation; production AWS Adapter remains pending M11-03B |
