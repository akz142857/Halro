...
| 内存里一条驻留 attempt（含 summary 与索引） | 约 1050 B | `runtime.ReadMemStats` 实测，见 `TestTheWindowCostsAKnownAmountOfMemoryPerAttempt` |
| `AttemptEvent` / `RequestSummary` 结构体本身 | 544 B / 208 B | `unsafe.Sizeof`；这是与字符串长度无关的地板 |
...
| **1 · 封存 WAL 段** | 最终档案，可完整重建 | 密封只读段，新 generation | **gzip 最快档** | 长期，不可删也不可搬走 | 4.08 → **约 0.73 GB/天** |
...
同一组数字也说明窗口本身在高吞吐下不够，真正的下一级杠杆是增量 checkpoint（S9，已完成）。详见第六章 S6。

S9 落地后，压缩这件事要重新问一遍：现在每 tick 只写一个 ≤ 4 MiB 的尾段，压它是毫秒级
而不是分钟级。但省下的仍然只是磁盘，而磁盘从来不是这一层的瓶颈——**结论不变，仍然不
压**，只是理由从「跑不完一个周期」换成了「没有要解决的问题」。
...
真正的下一级杠杆是**增量 checkpoint**：只序列化上次之后新增的部分，而不是每次重写全量。那会把每次 checkpoint 的成本从 O(窗口) 降到 O(增量)，与窗口长度脱钩。

**S9 · 增量 checkpoint** —— 已完成（2026-09-02）。

checkpoint 从「一个每 tick 整体重写的 blob」变成「一个 head + 一串只读记录段」。
head 里只有那些大小由窗口形状而非窗口长度决定的东西——总计、直方图、小时桶、在飞
请求、去重窗口，以及段索引；记录本身按账本顺序落在段里。一个 tick 只重写 head 和
那个尚未封口的尾段，尾段超过 4 MiB 就封口、开新段；被窗口裁到下界以下的整段直接
删除。因此一次 checkpoint 的代价是 O(自上次以来新增)，与驻留量无关。

这条路走得通的前提是 `PruneBefore` 已经确立的形状：记录只在尾部追加、只从头部整段
丢弃，中间永不改写。段因此天然不可变，restore 时按 floor 过滤即可精确复现裁剪结果
——一个被裁到一半的段不必重写，留在盘上直到它整段落到 floor 以下。

顺带修掉的两处：

- **编码不再持锁**。旧实现在 `a.mu.Lock()` 里做完整 `json.Marshal`，10 次/秒下等于
  每 60 秒里有 45 秒挡住 collector 写入。现在锁内只做小对象拷贝与增量切片，编码在
  锁外。
- **store 不再把 payload 套 JSON 信封**。`[]byte` 进 JSON 会被 base64，27 GB 的
  checkpoint 因此在写盘前先变成约 36 GB 字符串，外加 `bytes.Clone` 与 marshal 缓冲
  各一份全量副本。段现在按原样存进独立的 bbolt 桶，信封只剩下小小的 head。

**它不解决的**：驻留内存。控制台仍然读内存里的聚合，30 天 × 10 次/秒仍然是约 25 GB
常驻——那要把控制台改成从盘上分页，是另一个题目。S9 消掉的是每 tick 的 CPU、每 tick
的内存尖峰（实测约为 blob 的 4–5 倍）和随之而来的 RTO。

**格式变更与后果**：`checkpointVersion` 11 → 12，metadata schema 34 → 35。迁移 35
建桶并**清掉两个 Usage 派生物**（checkpoint 与 rollup 行），因此升级后第一次启动会
从账本完整重放重建它们——日志里是 `usage derivatives rebuilt from the ledger`。不需要
重新初始化数据目录。
...
**不删除任何东西，也还不能搬走。** 封存段的保留策略是「长期」：账务要能从完整历史
重建，所以段是永久归档，只是小 5.4 倍。因此 D6 的三条封存前置条件不再是滚动的前置
条件（滚动不丢任何东西），而是**压缩**的前置条件——一个已经完全进入 Parquet 归档并
越过 usage checkpoint 的代，才是可以安全压缩的代。

**「可外移」这一条已订正为「不可」（2026-09-03）。** 封存历史必须从 generation 1
起连续，`loadSegmentManifest` 现在强制这一点。理由是：一份被改成只列最近几代的清单
仍然有序、文件仍然齐全、`ledger verify` 仍然通过，而一次完整重放会据此从历史的一个
后缀重建余额、并且什么都不报——这正是账务权威绝不能有的失败形态。要真正支持搬走一
个代，需要一份「搬走了什么、它承载了多少余额」的持久记录，那份记录目前不存在。今天
能用的杠杆是压缩，把归档压到约 1/5.4。
...
9. （第三阶段）**活动** WAL 不再随运行时间单调上升，归档缩小到约 1/5.4；
