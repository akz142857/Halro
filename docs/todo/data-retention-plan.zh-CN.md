...
`docs/guides/operator-guide.md` 与最终失败列表页面说「可见窗口等于 Usage 保留窗口」。**这句话是错的**：可见窗口是无限的，`retention_days` 与之无关。应与第一阶段同时订正。
...
| **3 · Aggregate + checkpoint** | 控制台工作集 | JSON in bbolt | 窗口落地后再评估 | `console_window_days`，默认 30 | 27.8 GB → 压后 3.8 GB |
...
### 3.3 为什么 checkpoint 的压缩是收尾而不是解法

序列化本身已经是瓶颈（30 天约 45 秒/次），压缩再加 2.1 秒/天量级。**先有 30 天窗口，checkpoint 才从 27.8 GB 降到可接受；那之后再压到 3.8 GB 是锦上添花。** 顺序反过来，只是把「磁盘爆掉」换成「CPU 追不上」。
...
**S6 · checkpoint 压缩（可选）**
- 仅在窗口落地并观测到收益后进行；
- gzip 最快档，就地改格式 + 提版本号，旧 checkpoint 拒绝并重建。
