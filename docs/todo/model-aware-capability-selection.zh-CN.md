...
- 只有 Halro 内置目录与管理员明确触发后成功的固定协议验证可以让能力默认勾选；服务商返回的元数据一律需要管理员显式确认（见 §4.1）。
...
4. 未知模型默认零能力；普通路径通过显式、受控的能力识别建立证据，高级路径保留手动声明。
...
| `verified_probe` | 管理员显式触发、固定协议且无持久副作用的验证结果 | 是，受探测覆盖范围限制 | 是，仅自动选择 `supported` |
| `operator_declared` | 管理员对未知模型的显式声明 | 是，但证据保持 Declared，不能伪装成 Verified | 否 |
| `unsupported` | 已知不支持或无法证明 | 否 | 否 |

`provider_metadata` 是**外部输入**：上游返回什么，Halro 就会据此展示什么。如果它既能扩张能力又默认勾选，等于把“默认启用哪些能力”的决定权交给上游。因此它只能渲染为“可用 · 待确认”，必须由管理员显式勾选，且写入 Snapshot 的证据等级不得高于 `declared`。只有 `builtin_catalog` 与明确触发后逐项成功的 `verified_probe` 允许默认勾选。
...
1. 显示“该模型尚未收录”，不默认勾选任何核心操作；
2. 只有管理员明确点击“确认模型并识别能力”才启动受控检测；输入、搜索、选择、hover 与 blur 都不发 Provider 请求；
3. 目录或新鲜同目标缓存命中时零 Provider 调用，否则按单一 Binding 的固定协议逐项验证；
4. 只自动勾选 `supported`，管理员在普通路径只能关闭；
5. 无法检测时提供“声明模型用途”的高级入口，Evidence 保持 Declared；
6. 两条路径均保存为停用，完成精确 Deployment revision 的真实测试后才能启用；
7. 检测不预填 `LastTestRevision`，也不能把未执行的能力升级为 Verified。
...
- 只渲染模型目录支持的能力；
- `builtin_catalog` 精确命中继续零点击预选；未知模型只有显式检测后状态为 `supported` 的能力自动选中；`provider_metadata` 仍为“可用 · 待确认”；
- 管理员可以关闭，但不能添加目录外能力；
- 精确区分“视觉输入”与“图像生成”；
...
未知模型普通创建引用当前、未过期且同目标的检测 ID/revision，并通过
`capabilities` 提交推荐集合的收窄子集。后端重新校验 Provider、凭据、
Binding、模型、区域、目标指纹、TTL 与 revision；客户端不能提交 verified
evidence。
...
- 未知模型普通路径进入显式能力识别，高级入口保留手动声明；
- 首次引导和普通新增共用初始化路径。

门禁：已知单 Binding 模型可完整创建、测试、定价、启用、路由；未知模型只能从显式检测或高级声明获得能力。
...
- 已知模型自动选 Binding；
- 未知模型没有当前检测或高级声明时普通创建被拒绝；
- 当前检测只能用于同 Provider、凭据 revision、Binding、模型、区域和目标；
- 手动声明保持 Declared；
...
- 不支持能力不渲染；
- 管理员只能取消，不能添加；
- 仅 `builtin_catalog` 与成功的 `verified_probe` 能力默认勾选；`provider_metadata` 能力仍待确认；
...
- [x] 未知模型默认零能力；普通路径须显式识别，高级路径才是手动声明。
- [-] 内置目录与成功检测可默认勾选，上游元数据默认待确认。`builtin_catalog` 与 `verified_probe` 已有测试；`provider_metadata` 尚无 Adapter 构造该来源，规则继续 fail-closed，待首个实现时补实体验收。
...
- [x] 完整 Go、Race、Vet、前端测试、生产构建和浏览器验收通过。浏览器已在已登录的本机 Console 验证：未知模型输入只展示显式识别 CTA 与费用边界，未自动发起 Provider 请求，并可进入高级手动声明。
- [ ] 真实 Provider 能力证据已进入 `docs/verification/provider-real-matrix.md`。**未达成**。

计数：共 19 条 —— **17 达成 `[x]`、1 未达成 `[ ]`、1 阻塞 `[-]`**。标记即事实，计数由标记数出来，不要反过来信这句话。

`[-]` 是**阻塞**：规则已实现，但缺少可演示的对象，继续写代码也推不动它，因此**不计入未完成门禁**。目前只有门禁 4 属于这一类。剩下真正未完成的是门禁 19（真实 Provider 证据未取得）。逐项跟踪见 §17.4。
...
**当前 `schemaVersion = 24`。** 迁移 24 增加持久化模型能力检测、幂等与指纹索引桶；迁移 23（`deployment_snapshot_evidence_and_disabled`，随 #134）与前三道不同：它**回填**而不是拒绝。判据是值能不能被重建 —— 迁移 20、21 拒绝，是因为能力快照与 `legacy` 证据都无法在不编造主张的前提下补出来；迁移 23 的两个字段是记录中已有字段的纯函数，由写路径同一组 domain helper 计算，因此迁移后的记录与该记录重新保存的结果逐字节相同，并有测试断言这一点。
...
- **F2. 浏览器验收**已完成：在已登录的本机 Console 验证未知模型显式 CTA、调用/费用提示和高级声明逃生通道；未点击真实探测，避免产生未授权费用。
...
| ~~F2~~ | ~~浏览器验收（门禁 18 的最后一项）~~ | **已完成** | 已在已登录的本机 Console 验证显式 CTA、费用边界与高级声明路径；真实探测留给 F1 的计费门禁 |
...
**门禁现状（§15，共 19 条）：17 达成 / 1 未达成 / 1 阻塞。** 阻塞的是 §15-4，它在 §15 里标为 `[-]` 且**不计入未完成门禁** —— 规则已实现，缺的是可演示的对象。真正未完成的只有 F1（真实 Provider 证据）。

**剩下的没有一项是编码工作** —— 唯一未完成门禁 F1 只差精确 RC、计费授权和真实 Provider 凭据，另有一项 §15-4 处于阻塞、不计入。（这句话在 2026-08-09 的评审中被发现说早了：当时 §12.5 的目录不可用兜底前端测试还缺，那是编码工作。该测试已补，见上表 F3，因此这句话现在成立。）1.1.0 的工作另见
