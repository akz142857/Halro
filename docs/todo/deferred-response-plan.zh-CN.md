...
### 11. `Idempotency-Key` 是可选的

正文第五节把它标成「可选」，第一版实现却对每次提交都调 `idempotency.ValidateKey`，空字符串被拒
——于是不带这个头的提交一律 400。这是跑真实二进制发现的：单元测试全绿，第一次用 curl 打真实
实例就撞上了。资源端点要求这个头是对的（它们会在上游创建一个绝不能创建两次的对象），但延迟提交
就是同步路径的那次生成，而 `/v1/responses` 从来不要求这个头。现在只在非空时校验。

### 12. 多角色 review 抓到的两处缺陷（已修）

方案写完、门禁全绿、真实二进制端到端跑通之后的 review 又抓到两条，都是单元测试没有覆盖的状态机
分支：

**（a）项目到达 TPM 或并发上限时，出队的请求被判为 `failed`，而不是退回队列。** D5 明明写着
「拿不到就退避重试，记录保持 queued」，实现却把 `beginRequestRun` 的限流错误当成普通失败，连
密封的请求对象一起抹掉。也就是说：队列在它唯一被设计来吸收的那种压力下，会直接丢掉请求。修法是
识别三个限流错误码，把记录退回 `queued` 并保留输入对象；同时给调度器一个 `blocked` 标志，避免
退回之后立刻再取、再被拒的热循环。

**（b）进程正常关闭时，正在执行的请求被写成 `cancelled`。** 调用方没有取消过任何东西，却被告知
是自己取消的。根因是「调用方取消」和「进程关闭」到达 worker 时是同一个信号——一个被取消的
context。修法是把引擎的 run context 存下来，两者据此分辨：关闭写 `failed` +
`deferred_response_interrupted`，取消才写 `cancelled`。
