...
| Master key | provider-neutral `Unlocker` boundary across Runtime/init/bootstrap/Admin/Audit/Metrics/doctor/backup/restore/rotate; strict final `storage.master_key` File/Key-Slot schema with zero-I/O config check; File backend atomic create/replace, `0600`, no-follow load and wrong-key startup check; official AWS KMS Adapter with enforced Workload Identity sources, symmetric Encrypt/Decrypt, strict Key/account/region/endpoint/algorithm allowlists, versioned Encryption Context, canonical protected payload, typed errors and core-owned bounded retry; explicit AWS KMS initialization with independent verified Primary/Recovery Slots, atomic staged root-state publication, Primary-only Runtime/Bootstrap/Admin, audited break-glass Recovery and KMS-specific persistence/telemetry/heap Canary (real AWS evidence pending); persistent versioned keyring; offline COW rotation with per-Credential re-encryption, stable protected Audit key, Admin-session invalidation, compaction, idempotent crash bridge, repeated rotation, and nine kill-point recoveries |
| KMS extension boundary | accepted single-module/single-artifact AWS SDK layout backed by reproducible module/binary/container/build/test/cold-start/SBOM/vulnerability measurements; provider-neutral bounded `kms.Wrapper`, stable typed error taxonomy, authenticated fake KMS timeout/cancellation/fault contract, File-mode live metadata zero-call proof, CI-enforced Gateway/core dependency isolation, KMS-aware doctor/backup/restore/lifecycle, correlated provider-request Audit, low-cardinality Metrics/alerts, and host/deployment hardening baselines; production-ready remains blocked on the M11 real-AWS matrix, independent recovery drill, signed RC evidence, and four-party review |
...
| First-run experience | `halro start` safe config generation, fail-closed idempotent system initialization, atomic first-Admin/audit/completion commit, generated or absolute-expiry file-backed setup token, authenticated Audit-evidence replay, offline idempotent bootstrap, Kubernetes Init/Bootstrap/Verification Job and default-deny examples, and incremental `make start` build path |
