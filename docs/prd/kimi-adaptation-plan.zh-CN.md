...
## 17. tool_choice 判据第三版，以及路由前提的实测（2026-09-02）

§16 把「总是推理」接进路由之后，回过头验两件建立在**二手实测或推断**上的东西。四条
探针，三条确认、一条推翻了我自己上一轮的改动。

### 17.1 路由前提站得住（确认）

`ReasonsUnasked` 的标记、以及由它驱动的路由绕开，此前依据的是「k2.7-code 拒绝
`thinking:{"type":"disabled"}`」这条实测，**加上一步推断**：既然关不掉，那就总在推理。
后半句没人验过——而 MiniMax M3 刚刚示范过这两句不是一回事（文档说 `adaptive` 会思考，
实测两次都没返回 reasoning 项）。

裸请求，不带任何 thinking 成员：

```
kimi-k2.7-code            reasoning_content 非空,  reasoning_tokens 26
kimi-k2.7-code-highspeed  reasoning_content 非空,  reasoning_tokens 28
```

前提成立。`-highspeed` 顺带从「按同一条线推断」升级为一手实测——目录注释原本写明它没被
单独驱动过，现在改掉了。

### 17.2 tool_choice：从一个误拒换成了另一个误拒

§14.6 把判据从「按模型」改成「按思考状态」，依据是 §10.3 记录的实测。这次直接驱动：

```
kimi-k2.6 + thinking:{"type":"disabled"} + tool_choice:"required"
  -> 200, tool_calls, reasoning_content 为空
kimi-k3   + reasoning_effort:"high"      + tool_choice:"required"
  -> 200, tool_calls, reasoning_content 非空, reasoning_tokens 26
```

第一条确认了旧代码（按模型）的误拒是真的：K2.x 的 `required` 被一律拒掉，而那正是最普通
的可移植请求——没人要求推理，渲染器把思考关掉，Kimi 欣然接受。

**第二条推翻了我的修法。** kimi-k3 一边推理一边强制调用工具，毫无问题。所以「按思考状态」
同样是错的，只是错在另一行。两个版本各对了一半：

| 模型 | 思考 | 强制工具 | 实测 | 按模型 | 按思考 | 合取 |
|---|---|---|---|---|---|---|
| k3 | 开 | required | **200** | 放行 ✓ | 拒 ✗ | 放行 ✓ |
| k3 | 关 | required | （未测，两版都放行） | 放行 | 放行 | 放行 |
| k2.6 | 关 | required | **200** | 拒 ✗ | 放行 ✓ | 放行 ✓ |
| K2.x | 开 | required | 报错（§10.3） | 拒 ✓ | 拒 ✓ | 拒 ✓ |

真相是**合取**：K2.x 线 **且** 思考开着。第三版按这个改。

### 17.3 连带撤掉一条字段规则

§14.6 同时加了一条字段规则，把「深度 + 强制工具」在路由层整个绕开。现在知道 kimi-k3 上
这个组合能用，那条规则会**连它一起拿掉**——为了躲开一个失败组合而牺牲旗舰模型上一个能用
的组合，方向反了。

规则撤掉，拒绝留在渲染器里（正确收窄到非 k3），按既有方式记为 per-model 残留 —— 与输出
上限、推理拼写那两处同类。`docs/compatibility/endpoint-manifests.json` 里三处
`tool_choice` 申报随之撤回，申报文字改成实测后的说法。
...
### 17.6 第四版：两种形态是两条规则（同日）

上一节标记的那一格驱动了，**它确实冲突**：

```
kimi-k3 + reasoning_effort:"high" + tool_choice:{具名函数}
  -> 400  tool_choice 'specified' is incompatible with thinking enabled
```

同一个模型、同一个深度，`required` 是 200 而具名函数是 400。所以两种形态从来不是同一条
规则：

| tool_choice | k3 + 思考 | k2.6 + 思考 | k2.6 无思考 |
|---|---|---|---|
| `required` | **200** | 400 | 200 |
| 具名函数 | **400** | 400 | 未测 |

- **`required`** —— 合取：K2.x 线 **且** 思考开。k3 豁免。
- **具名函数** —— 只看思考开关，**所有模型**都拒。k3 不豁免。

第三版把两种形态一视同仁地套合取，于是放行了 k3 + 深度 + 具名函数，而它 400。那是我上一节
自己标记为"可能拒得太少"的那格，标对了。

**一个好的副作用**：具名函数这一半既然与模型无关，就是 profile 层能表达的，于是搬回了字段
规则——「深度 + 具名函数」现在在**预留之前**被路由绕开，不再是预留之后的 400。`required`
那一半留在渲染器里，因为它确实分模型，而按 profile 表达它就得连 k3 能用的请求一起拿掉。

`docs/compatibility/endpoint-manifests.json` 里 `tool_choice` 的申报第三次改写，这次说的是
两种形态各自的处置。
