...
- 专业化 Work Unit 详情 Drawer：标题使用短 ID；生命周期采用“创建 → 当前/关闭”节点并将冻结口径降为上下文；业务结果独立展示；调用明细按状态与请求、模型路由、核心指标、费用证据四层组织；长标识可复制，选中态和状态均提供非颜色表达；
...
最终门禁：TypeScript typecheck 通过；Vitest `44 files / 556 tests` 通过；生产构建、bundle size 与 `29 files` artifact secret scan 通过。连续两次生产构建的 embedded bundle 内容哈希一致（`7807ad956139edb3eb1770ddaa293fdb26d2eb251d52cec428a4215c64a02964`）。本次仅修改前端、文案和生成 bundle，没有 Go 源码变化，依据仓库“run what the change can affect”规则未重复运行 Go / race 门禁。
