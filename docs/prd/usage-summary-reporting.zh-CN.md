...
- **区间上限按桶数，不按月数**：day 最多 400 个桶、month 36 个、year 10 个，超出返回 400 而不是
  静默截断。第一版把上限写成「24 个月」，而按年视图的默认区间是 3 年 = 25 个月——默认值违反了
  自己的上限，按年视图开箱即错。上限和默认区间是同一个决定的两半，必须一起测（§八 已加）。
...
- **默认区间可服务**：每个粒度的默认窗口都必须落在它自己的桶数上限内——一个不可服务的默认值
  会让该视图只剩一个报错页。
