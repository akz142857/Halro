...
整改过程中原报告的四条结论被修正，另外发现**十二项**报告里没有的问题（全部已修完）。最后四项来自 2026-08-07 对着跑起来的控制台走了一遍——不是读代码读出来的，是用出来的。
...
> **已过期，不要再引用这张表做决策**（2026-08-12 标注）。2026-08-11 的
> v1.0.0 放行评审 A6 重新量过：跨 admin/非 admin 边界符号从约 11 个升到约 27 个
> （≈2.5×），五把 admin 锁里 `adminTopologyMu` 已越出边界
> （`internal/app/activation_state.go`、`internal/app/admin_providers.go`），
> `internal/app` 非测试行数 17,629 → 21,079。结论"1.0.0 前不拆"仍然成立，但
> 下面这张表的两个支撑事实（锁零外部引用、状态可整体搬走）已不再为真。
> 详见 `docs/review/260811/releases.1.0.0/release-1.0.0-report.md` §9.2 第 2 条。
> 1.0.0 之后的第一个间隔必须重新量一次，而不是沿用这张表。
...
| 例外操作长在每一行里 | `web/src/pages/UsagePage.tsx` | **已修** `4f4f542`。「用量与调用」的每行成本列都挂着「调整成本」，让一张日志看起来像可编辑的表。它其实是例外路径——要填原因、证据 SHA-256、再做 step-up 重认证。收进已默认折叠的「计价证据」里：那是"已经在看这个数怎么算出来的"的状态，也是唯一适合动它的状态。**用户提出的判断**：没人会对着成千上万行逐条调整。判断成立，但它指向的不是"该删掉"，而是"位置错了" |
| 调整历史接口从来没有客户端调用 | `internal/app/runtime.go:1154` `GET .../cost-adjustments` | **已修** `4f4f542`。端点从调整功能上线起就注册着，`web/src/api.ts` 里没有对应函数——控制台能追加一条修正，却看不到自己追加过什么。现接在同一个折叠区里，按展开加载而不是按行加载（一页 100 条，真正被点开的几乎没有） |
| 「调整后超预算」信号点不进去 | `internal/app/admin_usage.go` `dashboardGovernance` | **已修**（本轮）。信号只报一个计数，而它明明在循环里就知道是哪些项目——报出来的却只有数字。后端补 `adjustment_over_budget_items`（项目 ID/名称/已提交/预算/调整净额），前端把每个项目渲染成一条链接直达该项目的调用列表。一个拉响警报却不说是谁的信号，等于让操作员自己去翻 |
| `navigate()` 忽略查询串 | `web/src/navigation.tsx` | **已修**（本轮）。早退判断是 `window.location.pathname === path`，而好几个目的地把主语放在查询串里（request_id、project_id）。后果是"在同一页面换一个筛选去看"整个变成空操作——包括把筛选清回全量列表。改为比较 `pathname + search`。这是做上一条时撞上的既有缺陷，不是新引入的 |
