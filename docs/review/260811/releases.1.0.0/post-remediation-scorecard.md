...
**封顶前加权总分 7.35 / 10；封顶后最终分 6.5 / 10**（宽读下 5.0，见 §3）。
...
|---|---:|---:|---:|---|---|---|
| D1 安全性 | 0.20 | 6.5 | **7.5** | V1 那条 fail-open 不是"补一句裁决"而是代码层关死：`internal/redaction/engine.go:433-444` 与 `internal/tokenguard/manager.go:311-322` 对具名策略缺失一律拒绝（空 ID 仍放行，因为那是"该项目没有策略"这个决定），`internal/gateway/service.go:184-220` 在授权阶段一次性拒绝这类 Project 并返回 503 `configuration_stale`；两条花费凭据的侧信道改为 POST 走 `requireAdminMutation`（角色 + CSRF + Origin），`requireCredentialedAdminRead` 整个删除、不与替代品并存；argon2 全局信号量 + 实测 | R-15 未闭：`docs/verification/security-review-v1.md` 的四道门仍未关，六个新子系统（kms / masterkey / hostsecurity / bearercred / modelcatalog / bedrockmantle）的增量安全评审未做，`internal/kms/awskms/adapter.go:31` 的 `LoadDefaultConfig` 威胁评审仍未发生 | 关掉四道门或显式声明覆盖边界；补一次增量安全评审 |
...
| **加权总分** | **1.00** | **6.21** | **7.35** | 触发的封顶规则：**G4/G7 未通过 → 总分封顶 6.5**（rubric §5 第 2 行）。**封顶咬合**，最终分 = **6.5** | | |

### 2.1 两个数字的差距，这次说的是相反的事

报告那轮加权分 6.21、封顶 6.5，差 0.29，结论是"封顶几乎没起作用——**问题是弥散的**"。

这轮加权分 7.35、封顶 6.5，差 **0.85**，封顶明确咬合。按同一条读法，这说明
...
| G7 | 未通过 | **仍未通过** | 6 条 🔴 无一有书面裁决。代码侧复查确认原样：`syncUsageAdmin` 仍用 `request.Context()`（`internal/app/admin_usage.go:567-573`）、`requireDestructiveStepUp` 仍只挂在 DELETE 上、`internal/sourcelimit/limiter.go:112` 无取舍注释。**其中 #13 的第三子项已因勘误而消解**（见 §5） |
| G8 | 未完成 | **通过** | 发布说明 20 条已知限制，⚠ 三条按要求改写而非固化 |
| G9 | 通过 | **维持通过** | `release_24h` 仍未归档，报告已明确该项在 G9 外 |

**宽读记录**：若把 B3-01 计为 D8 的 CONFIRMED P0，则 D8 封顶 4（加权分变 7.17）、
...
- **Admin 面**：TOTP 2FA、CSRF、revision 校验、破坏性操作 step-up。
  step-up 目前挂在 DELETE 上，两个 PUT 未挂——**这是 G7 待拍板的取舍之一**。
...
| 二 | G7 剩余条目的书面裁决：`syncUsageAdmin` 的 ctx 语义、两个 PUT 的 step-up、溢出预算随 `maxTracked` 缩放、rc.1 根因、能力选择的浏览器验收与真实 Provider 证据、`security-review-v1.md` 的四道门 | **G7**，其中最后一条同时抬 D1 | 取决于拍板，本身不是工程量 |
