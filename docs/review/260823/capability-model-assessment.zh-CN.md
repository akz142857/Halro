...
三条论点里仍未关闭的只剩一件事：**"一份平台描述"本身**——六处登记合并成一处，受包依赖方向所限（能力上限在 `domain`、Operation→Primitive 绑定在 `provider`、字段规则是对 `semantic` 请求的闭包在 `compatibility`、适配器构造在 `app`）。它需要新开顶层包或代码生成，且不再有静默风险作为催办理由。
