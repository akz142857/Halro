...
| 论点 | 状态 | 已关闭的 | 仍然存在的 |
|---|---|---|---|
| 1 全能力模型 | **部分** | ①② 命名统一、合成一张配对表、漏配对会红；一个事实（能取图）从 ③ 移入 ① 并因此在控制台可见 | **四种表示一个都没消失**；`ProviderCapabilities` 仍有 4 份拷贝（21 字段 × Go 3 + TS 1）、3 个手写逐字段转换；按名字枚举能力的 switch/map 仍有 6 处 |
| 2 独立适配域 | **完全没做** | — | 加一个 Provider Profile 仍要在 6 个文件登记；清单仍是 20 端点 × 65 条手写 coverage |
| 3 服务端构建 | **部分** | 字段级限制已发布到控制台，部署表单在勾选处显示 | `types.ts` 的 21 字段接口纯手工镜像且**无任何守卫**；两个 locale 的标签表同样无守卫；唯一的守卫（分组完整性）是构建期 golden 而非运行时契约 |

最能说明论点 1 未完成的是实施本身的证据：加 `fetched_image` **一项**能力动了 20 余个文件，其中第 4、第 5 处按名字枚举的地方（`admin_deployments.go` 的 `capabilitiesEnabledByName`、`models.go` 里逐字段 OR 的绑定汇总）是靠测试失败才发现的。真有一个统一模型时，加一项能力应当只动一处。

P1 关闭的是 §1.2 描述的裂缝——① 与 ② 之间靠手写配对连接。它没有触及 §1.1 的四种表示本身，那是 P2 的范围。
