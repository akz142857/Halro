...
当前状态：**整改代码与本机全量门禁已完成；Owner 条件式 GO，精确 SHA 的 CI 与发布彩排执行中**
...
发布责任人已在 [`v0.8.0 assessment`](../../verification/assessments/v0.8.0.md) 中逐项接受这些
残余证据边界并请求发布。该接受不把未执行项目改写成通过；条件式 GO 仍要求同一精确 SHA 的
普通 CI 与 release dry-run 全绿。
