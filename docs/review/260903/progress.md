...
完成:四条 P0、六条 P1、十一条 P2 已修、两条 P2 部分修复、一条 P2 未做,
四条「说法与实现不符」,以及若干 P3。P10/P14 的后续整改另记在下表及后续评估。
...
| P14 | `performance-baseline.md` 对路由解析差 12 倍 | **已修**。同机 Go 1.26.6、相同 fixture 对比 v0.5.0 与候选版本,10 项各 8 组样本;原始输出、源码指纹及 benchstat 已归档。旧表标为历史数据,限流 +9.82% 与 WAL 累计分配 +5.58% 如实记录;见后续评估 |
| P5 | 汇总端点两段读竞态 | 未复查(本轮 `checkpoint.go` 是新代码,核心逻辑角色确认 checkpoint 与 rollup 同事务同前缀) |
| P10 | `usage rebuild-summary` 用未认证 WAL 重放 | **已修**。离线 usage 的四个命令改为逐帧认证活动 WAL 与全部封存世代,并核对可信 checkpoint;认证失败前不创建 exporter、不打开可写 metadata。新增回归在旧实现上证实活动 WAL 和封存段都能被篡改后重建,因此影响并不只限于开启 sealing 的实例。见 `docs/verification/assessments/v0.6.0-followup.md` |
...
从 260901 结转的 **P10 已在后续整改关闭**,覆盖活动 WAL 和封存段,不以 sealing
开关作为认证开关。**P14 也已关闭证据缺口**;性能变化及测量边界见上表及后续评估记录。
