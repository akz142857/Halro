...
清单共 32 项，**已完成 12 项**（逐条到代码里核实过，非仅凭本文件记录）：

| 档位 | 完成 / 总数 | 提交 |
|---|---|---|
| P0 | **5 / 5** | `897894e` |
| P1 | **7 / 10** | `8e373fb`、`e41446a`、`9c038b5` |
| P2 | 0 / 17 | — |

剩余三项 P1（P1-7/14/15）全部需要前端改动，是继续推进的下一个批次。
...
**剩余三项 P1-7/14/15 全部需要前端改动**，攒成一批做，只重建一次 `internal/webui/dist`。P1-7 尤其：给网关密钥铸造等端点加 step-up 意味着前端对话框要加二次认证字段，后端单独改会直接让控制台的相应操作开始返回 401。
