...
>
> 编号沿用 260807.md 第九章的修复清单，另单列第七章 BUG 清单的对照。最后更新：2026-08-07，复核账目、订正计数，并修完第七章里此前无人跟踪的四条。

## 一句话状态

第九章修复清单共 33 项（P0 五项 + P1 十项 + P2 十八项），**全部结项**（32 项完成 + 1 项经论证后不做）（逐条到代码里核实过，非仅凭本文件记录）。**第七章的 11 条 BUG 是另一套编号**，此前无人跟踪，现已单列对照表（见"第七章 BUG 清单的独立对照"），11 条也已全部结项。

| 档位 | 完成 / 总数 | 提交 |
|---|---|---|
| P0 | **5 / 5** | `897894e` |
| P1 | **10 / 10** | `8e373fb`、`e41446a`、`9c038b5`、`4bb3fc2` |
| P2 | **17 / 18**（P2-3 不做） | `093ff1a`、`70d107d`、`c9528ab`、`18ff5d5`、`863adc2`、`97befaf`、`25f3939`、`9b31e62`、`cd1f32e` |

P0、P1、P2 三档全部结项。**但"结项"不等于每条的每一半都做了**：多条在做的过程中留了明确取舍，逐条记在各自小节里，文末"仍开着的事"是它们的汇总。其中两处此前**被本文件记成转交给了另一个编号，而那个编号并没有接住**——P1-6(b) 的 argon2 信号量（转给 P2-32）、P0-5 的 `WithoutCancel` 与 `applyMu`（转给 P1）——顺着读会以为已经有人做了。
...
`Replay` 的错误分类已完成。清单里同项的第二半——`syncUsageAdmin` 改用 `context.WithoutCancel` + 独立超时——**未做**：`Replay` 不再毒化状态后，请求取消已经不产生持久后果，`WithoutCancel` 从"必须"降为纵深防御。连同对抗验证指出的 `applyMu.Lock()` 不感知 ctx（并发 admin 请求时后到者会阻塞整个 leader 的 replay 时长），当时写的是"一起移到 P1 处理"。

**订正：P1 并没有接这两件事**，十项 P1 里没有一条覆盖它们，`admin_usage.go` 至今没有 `WithoutCancel`。它们不是被 P1 做掉了，是被降级后没有再挂到任何编号下——已收进文末"仍开着的事"。
...
**2.（已修，`c9528ab`）CI 和 `make check` 都没有 gofmt 门禁。** 上一条能在 `main` 上存活正是因为这个——CLAUDE.md 写了"run gofmt on changed files"，但没有任何东西强制。已补 `fmt-check` 目标（`gofmt -l ./cmd ./internal ./tools` 非空即失败）并挂进 `make check`，现在的序列是 `fmt-check test race vet frontend-test observability-check`。
...
| P2-29 | 视觉六项 | **完成** `9b31e62`（ratchet 未扩展到 TSX 内联样式，见下） |
...
**P2-29 的 ratchet 没有扩展到 TSX 内联样式。** 六项视觉改动都做了，基线也降回 748，但 `design-system.test.ts:152` 的正则只读 `styles.css`，清单要求的"扩展到 TSX 内联样式"没做。当下无暴露面——全仓 `style={{...}}` 里带 `px` 的写法是 0 处——所以它挡不住的是**将来**第一个写进 TSX 的裸值，而不是现存的某个。归入下面"仍开着的事"。
...
三档全部结项。33 项里 32 项完成，1 项经论证后不做（P2-3 的退役占位）。

> 计数订正：此前这里写"32 项里 30 项完成，2 项不做"，三处都错。总数是 5 + 10 + 18 = **33**（P2 有十八项：P2-3 加 P2-16…P2-32）；不做的只有 P2-3 一项，被记成两项是因为把 P2-19 也算了进去——而本轮的 P2-19 是"ADR 0013 状态改 Superseded"，已由 `c9528ab` 完成；上一轮那条决定不做的"`internal/app` 拆 `adminapi`"是 260805 的编号，不属于本清单。
...
裸尺寸值回到 **748**，与上一轮基线 `126fb32` 持平，ratchet 基线同步降到 748（此前写宽了 4 格，且已被后续提交吃光）。现在任何新裸值都会让 `styles.css` 的测试红（TSX 内联样式仍在 ratchet 之外，见 P2-29 一节）。

### 收尾之后还有一次改动（`6fae50e`，本文件此前漏记）

收尾提交 `f30ea04` 之后又落了一个前端提交，属于本轮范围，补记于此：

- `datetime-local` 自带固定高度/内边距/字号，两个时间筛选框和同一行的文本框、下拉框不齐。现在改为跟它们取同一套，只单独处理浏览器自绘的日历按钮配色。
- **P1-15 的"界面标注所用时区"换了位置。** 原来标在筛选行旁边，现在移到页头（`web/src/Layout.tsx:104`），因为控制台头部本来就要写这句，重复一遍只是挤占了筛选行。**用记账时区解析输入这半（真正的正确性修复）没有动**，仍在 `UsagePage.tsx` 的 `zonedInputToISO`。以后核 P1-15 时不要在 UsagePage 里找那个标注。
- 顺带修了页头那句的读法：时区此前和前面两个断言共用同一个斜杠分隔符，读起来像第三个断言而不是"标签: 值"；分隔符加了 `aria-hidden`。

## 第七章 BUG 清单的独立对照（此前无人跟踪）

修复清单跟的是 260807.md **第九章**的 33 项编号；第七章的 11 条 BUG 是另一套编号，两者之间从来没有映射。#1~#4 因为进了 P0/P1 才被跟到，剩下 7 条既不在任何编号下，也不在本文件里——低危 BUG 就消失在两份清单的缝里。补一张对照表，逐条到代码里核实过：

| # | 严重度 | 状态 |
|---|---|---|
| 1 | 中 | **按 1.0.0 策略不修**（= P2-3，见上）。含 kind=6 帧的 WAL 打不开；V-2 论证现实受害者集合几乎为空 |
| 2 | 高 | **已修**（= P0-4） |
| 3 | 中 | **已修**（= P0-4） |
| 4 | 中 | **已修**（= P1-12） |
| 5 | 中 | **已修**：`pullAnchors` 现在明确不持 `e.mu` 做网络与文件写，注释已写明 |
| 6 | 低 | **已修**（本轮）：证人文件加 8 MiB 上限 + 单代轮转 |
| 7 | 低 | **已修**（本轮）：`cursor.Delete()` 后 `Next()` 跳元素 |
| 8 | 低 | **已修**（本轮）：`VerifyAuditAnchors` 不再把整条链的哈希装进内存 |
| 9 | 低 | **已修**（本轮）：`Records=0` 的伪造锚点不再恒判 agree |
| 10 | 低 | **已修（修法是删除，见下）**：口令清零 |
| 11 | 低 | **已修**：`adminStepUp` map 现按 4096 上限裁剪过期条目 |

### 本轮四条的修法与证据

**#7 的定性被实测推翻了一半。** 报告说"`cursor.Delete()` 后 `Next()` 会跳过一个元素，裁剪可能欠删"。第一个测试（把环撑到 retention 之上、一次删 6 个键）**在缺陷代码上直接通过**——因为 `Cursor.node()` 不把 node 写回 `c.stack`，游标在事务内仍按**页**迭代，`node.del` 只改内存 node，页没变所以不跳。跳元素只在 `ref.node != nil` 时成立，即**同事务里那张叶页已被 `Put` 物化**。构造这个条件（稀疏环，全部键同页）后复现为 `remaining=[2 4 6 1006 1007]`，隔一个留一个。

所以这条是**真实缺陷但当前不可触发**：`AppendAuditAnchor` 强制 `Sequence == last+1`，每次只有一个键过期，而写入键在高位、过期键在低位，1000 条时不同页。降低 retention 常量、或恢复一个更长的环，都会让它立刻生效。已改为先收集再 `bucket.Delete`（键要 `bytes.Clone`，删除会重排它们指向的内存）。两个测试都留下了：同页那个是钉住缺陷的，跨页那个钉住常规路径。

**#9 的攻击面确认成立。** `hashBySequence[anchor.Records]` 对不存在的键返回零值 `[32]byte`，所以 `Records: 0` + 全零 `LastHash` 的伪造行拿到 `agree`——实测缺陷态输出 `Outcome:"agree"`。发射端在 `summary.Records == lastEmittedRecords` 时直接返回，空链永不发射，故真锚点的 `Records` 恒 ≥ 1，改成 comma-ok 判存在不会误伤。这条值钱的地方在于：agree 本该是**没有审计密钥就伪造不出来的那个判定**，而它此前不需要任何密钥。

**#6 的修法要连读取侧一起改。** 只给 deadman 加轮转，`verify-anchor` 就会把退休那一代的全部序号报成 gap——把完整的记录判成不完整，和把不完整判成完整是同一种错。所以 `LoadAuditAnchorsFile` 同时读 `.1` 并置于前，缺 `.1` 正常、缺主文件仍报错。轮转用 rename 不用 truncate，退休那代保持完整。上限 8 MiB ≈ 四万条锚点，按 ADR 0015 默认发射率是数年。

**#10 的修法是删除，不是再加一层清零。** 口令从 JSON 解出来就是不可变 `string`，`net/http` 的读缓冲、解码器缓冲各还有一份，都不归这段代码所有、也无法覆写。把它复制成 `[]byte` 再 `clear` 掉的那一份，恰好是唯一一份反正就要变成垃圾的——**缩短不了任何暴露窗口，却读起来像密码已经从进程里擦掉了**。所以移除这个动作，把真实情况写在 `adminauth.VerifyPassword` 的文档注释里（它是共享边界）。

同一套动作在仓库里共有 **10 处**，报告只点了 `admin_stepup.go` 一处；只修那一处会让错的构件继续留在其余九处旁边，所以一并处理了。`VerifyPassword` 内部对**自己派生**的 candidate hash 的 `clear`、以及各处对 `PasswordHash`/`PasswordSalt` 的 `clear` 都保留——那些是本代码独占的值，清零是真的。

> 这条是判断，不是纯粹的缺陷修复：它删掉的是一个无效的防御动作，而不是补上一个有效的。若要让清零变成真的，得自己接管请求体缓冲并绕开 `encoding/json` 的字符串——那会为一条低危问题引入一条自定义解码路径，没有做。

## 仍开着的事（结项不等于每条的每一半都做了）

各条的取舍已分别记在上面，这里只做汇总，便于下一轮直接接手。除前两条外，其余都是当时明确论证过的取舍并原地记了"未做"，不是遗漏。

| 出处 | 仍未做的部分 | 性质 |
|---|---|---|
| **P1-6(b)** | **argon2 并发信号量**——代码侧对内存放大仍无界，只被 k8s 的 1Gi limit 兜着 | **曾被记成转交 P2-32，而 P2-32 只做了文档与内存上限**，见 P1-6 小节 |
| **P0-5** | `syncUsageAdmin` 的 `context.WithoutCancel` + 独立超时；`applyMu.Lock()` 不感知 ctx | **曾被记成转交 P1，而 P1 十项里没有一条覆盖它**；`Replay` 不再毒化状态后本身已降为纵深防御 |
