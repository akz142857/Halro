...
创建路由时，「公共模型别名」是一个组合框：选中已有别名即给它加一个目标，控制台会就地列出
该别名当前的目标与优先级；输入新名字则新建一个别名。重复指向同一部署、以及与该别名已启用
...
**这条校验只按部署去重，不等于按故障域去重。** 两条 Deployment 记录完全可以指向同一个
Profile Binding 与同一个上游模型，指向它们的两条路由仍然是一个凭据、一个配额，而校验放行。
要堵住那一层需要在部署写入路径上按「服务商 + Binding + 上游模型」唯一，那是一条关于「部署
是什么」的规则，尚未裁决。

**升级注意**：如果数据目录里已经存在一对「同别名、同部署、都启用」的路由（本次校验之前
建的，或从旧备份恢复的），此后对其中任何一条做无关编辑（例如只改优先级）都会被这条校验
拒绝，错误信息指向的是另一条你没动的路由。出路是先把其中一条改成禁用——`enabled: false`
会整体跳过这段校验——或者直接删除它（删除路径不走这条校验），然后再编辑保留的那条。
