...
Usage 页的失败计数可以点开成一份最终失败的请求列表——**每个请求一行**，不是每次尝试一行，所以卡片上的数字和列表长度不会互相矛盾。一个请求在第一个目标上失败、在下一个目标上成功，它不是失败请求，尽管它留下了一次失败的尝试。

每一行用你的语言给出失败分类，旁边是上游状态码，「下一步查什么」收在展开项里。有两种情况这一行不带服务商上下文，它们的含义不同：

- **请求从未调用上游**——被预算、熔断器、并发、Token Guard、能力过滤或脱敏拒绝，行内会说明是哪一种；
- **上游答了，请求在那之后才失败**——答案无法渲染，或出站脱敏拒绝了它。没有可指认的上游失败，所以不显示。

还有一种是记录写在这些字段加入之前，与「上游没给」不是一回事，界面会分开说。

若打开了 `gateway.failure_capture`，被捕获的失败请求可以展开对照 Gateway 收到的请求、Halro 规范化后的请求，以及上游返回或传输错误。打开这一项会写一条审计记录：这是控制台里唯一展示调用方所写内容的地方。

### 7.2 控制台窗口

「设置 → 实例」决定请求与失败列表能回看多久。初值来自 `usage.console_window_days`，之后由这个运行时设置决定；Parquet 归档有自己更长的保留期，所以缩短它改变的是控制台能显示什么，不是已经留下了什么。缩短需要确认，因为落在新窗口之外的记录会在下一次维护时从内存里裁掉。

### 7.3 延迟取回

项目允许时，调用方可以用 `background: true` 提交 `POST /v1/responses`，立刻拿到一个 id，稍后再来取答案。开关在项目页。答案密封落盘，最长保留 24 小时，首次取回后再保留 15 分钟——它对这个实例的数据目录意味着什么，见运维手册。
