...
**这把 Key 不一定能列。** 枚举是一项单独授予的权限：Key 必须同时带 `inference`
和 `discovery` 两个 scope，只有 `inference` 的 Key 照常调用管理员告诉它的别名，
但列出时返回 `403 gateway_key_scope_denied`。这不是缺陷——泄漏一把 Key 时，能调
用什么和能看到整份别名菜单是两件不同的事，所以由管理员按 Key 决定。拿到 403 时
应当向管理员申请这个 scope，而不是重试。

这条端点也和推理端点受同一层保护：管理员刚改完配置、运行快照还没跟上时，它同样
返回 `503 configuration_stale` 并带 `Retry-After`，不会因为"只是读配置"就放行。

它还与推理端点用不同的限流：这条路径不消耗 Project 的 RPM/TPM，而是受一道固定
的内建上限约束（每把 Key 每分钟 600 次，每个 Project 每分钟 5,000 次），超出时
返回 `429 rate_limit_exceeded` 并带 `Retry-After`。启动时列一次的客户端永远碰
不到它。
