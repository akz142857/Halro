...
import { GovernanceFirstRunGuide } from "./run-governance/GovernanceFirstRunGuide";
...
import { governanceContextFromSearch, isGovernanceDataStale, localDateValue, rememberedGovernanceProject, rememberGovernanceProject, type GovernanceView, type ResultsView, writeGovernanceContext } from "./run-governance/governance-state";
...
export function RunGovernancePage() {
  ...
  const [resultsEntry, setResultsEntry] = useState<ResultsView>("analytics");
  ...
  const keys = useQuery({ queryKey: ["project-keys", projectID], queryFn: () => api.keys(projectID), enabled: Boolean(projectID), refetchOnWindowFocus: true });
  ...
  const refreshAll = async () => {
    await Promise.all([projects.refetch(), workUnits.refetch(), runs.refetch(), definitions.refetch(), outcomes.refetch(), keys.refetch(), selectedDefinition ? summary.refetch() : Promise.resolve(), selectedRun ? attempts.refetch() : Promise.resolve()]);
  };
  ...
  const isRefreshing = [workUnits, runs, definitions, outcomes, keys, summary, attempts].some((query) => query.isFetching);
  ...
  function selectView(next: GovernanceView) {
    if (next === "results") setResultsEntry("analytics");
    updateContext({ view: next, workUnitID: next === "work-units" ? workUnitID : "", runID: next === "work-units" ? runID : "" });
  }
  ...
  // Historical lifecycle evidence is enough to retire the automatic first-run
  // guide. Do not make it reappear later merely because an operator rotated or
  // revoked the keys that originally completed the integration.
  const firstRunComplete = Boolean(
    definitions.data?.items.some((item) => item.enabled)
    && workUnits.data?.items.length
    && runs.data?.items.length
    && outcomes.data?.items.some((item) => !item.provisional),
  );
  // A single governed project is selected automatically. Do not flash the
  // project-missing guide for the render between the project list arriving and
  // that selection effect running.
  const guideReady = projects.isSuccess && (projectID
    ? Boolean(workUnits.data && runs.data && definitions.data && outcomes.data && (keys.data || keys.isError))
    : governanceProjects.length !== 1);
  const showGuide = guideReady && (guideRequested || (view === "overview" && !firstRunComplete && !guideDismissed));
  const pageAction = <div className="governance-page-actions" role="group" aria-label={t("runGovernance.actions")}>
    <button type="button" className="button ghost governance-page-action" onClick={() => { setGuideRequested(!showGuide); setGuideDismissed(showGuide); }}>
      <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M4 5.5h16v13H4zM8 9h8M8 13h5" /></svg>
      <span>{t(showGuide ? "runGovernance.firstRun.hide" : "runGovernance.firstRun.open")}</span>
    </button>
  ...
  return <>
    ...
    <section className="governance-shell">
      ...
      {showGuide && <GovernanceFirstRunGuide projectID={projectID} projectEnabled={Boolean(selectedProject)} definitions={definitions.data?.items ?? []} keys={keys.data?.items ?? []} keysUnavailable={keys.isError} workUnits={workUnits.data?.items ?? []} runs={runs.data?.items ?? []} outcomes={outcomes.data?.items ?? []} onOpenDefinitions={() => { setGuideRequested(false); setGuideDismissed(true); setResultsEntry("definitions"); updateContext({ view: "results", workUnitID: "", runID: "" }); }} />}
