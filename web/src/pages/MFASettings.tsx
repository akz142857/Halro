...
export function MFASettings({
  ...
  const refreshMFA = async () => { await queryClient.invalidateQueries({ queryKey: ["mfa"] }); };
  const refresh = async () => { await refreshMFA(); await queryClient.invalidateQueries({ queryKey: ["session"] }); };
  ...
  const confirm = useMutation({ mutationFn: () => api.confirmMFAAuthenticator(enrollment!.id, confirmCode), onSettled: () => setConfirmCode(""), onSuccess: async (value) => {
    const codes = value.recovery_codes || [];
    setRecoverySaved(false);
    setRecoveryConfirmed(false);
    setRecovery(codes);
    onRecoveryCodesPendingChange?.(codes.length > 0);
    setEnrollment(null);
    setName("");
    await refreshMFA();
    // The API guarantees codes for the first authenticator. Do not leave a
    // required account trapped if an older server returns no codes, however.
    if (codes.length === 0) await queryClient.invalidateQueries({ queryKey: ["session"] });
  } });
  ...
  const regenerate = useMutation({ mutationFn: () => api.regenerateMFARecoveryCodes(regenerateState.password, regenerateState.code), onSuccess: async (value) => { setRegenerateState({ open: false, password: "", code: "" }); setRecoverySaved(false); setRecoveryConfirmed(false); setRecovery(value.recovery_codes); await refreshMFA(); } });
  ...
  const finishRecovery = async () => {
    ...
    setRecoverySaved(false);
    setRecoveryConfirmed(false);
    onRecoveryCodesPendingChange?.(false);
    // Confirmation already activated MFA on the server. Update the local gate
    // synchronously so a failed session refresh cannot strand the operator on
    // the restricted page after the one-time codes have been acknowledged.
    queryClient.setQueryData(["session"], (current: Record<string, unknown> | undefined) => current ? { ...current, mfa_setup_required: false } : current);
  ...
  return <section className="panel settings-card mfa-settings" aria-labelledby="mfa-title" aria-busy={status.isPending}>
    ...
    {renameState.id && <form className="settings-form credential-form action-panel" aria-label={t("settings.renameAuthenticator")} onSubmit={(e) => { e.preventDefault(); rename.mutate(); }}><Field label={t("settings.deviceName")}><input autoComplete="off" autoFocus required maxLength={64} value={renameState.name} onChange={(e) => setRenameState((v) => ({ ...v, name: e.target.value }))} /></Field>{rename.isError && <ErrorState error={rename.error} />}<div className="form-actions"><button className="button primary" disabled={rename.isPending}>{t("common.save")}</button><button type="button" className="button ghost" onClick={() => setRenameState({ id: "", name: "", revision: 0 })}>{t("common.cancel")}</button></div></form>}
