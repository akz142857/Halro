...
export function WorkUnitExplorer({
  ...
  return <section className="governance-workspace" id="run-governance-panel" role="tabpanel" aria-labelledby="run-governance-tab-work-units">
    ...
    {selectedUnit && <Modal drawer title={`${t("runGovernance.workUnit")} · ${selectedUnit.id}`} onClose={onCloseDetail}>
      <div className="governance-detail">
        <section><div className="governance-detail-heading"><h3>{t("runGovernance.lifecycleAndOutcome")}</h3><WorkUnitStatusBadge status={selectedUnit.status} /></div><dl className="governance-detail-list"><div><dt>{t("runGovernance.createdAt")}</dt><dd>{dateTime(selectedUnit.created_at, "full")}</dd></div><div><dt>{t("runGovernance.closedAt")}</dt><dd>{selectedUnit.closed_at ? dateTime(selectedUnit.closed_at, "full") : "—"}</dd></div><div><dt>{t("runGovernance.frozenDefinitions")}</dt><dd>{selectedUnit.outcome_definitions?.length ? selectedUnit.outcome_definitions.map((item) => `${item.id} · v${item.version}`).join(", ") : "—"}</dd></div><div><dt>{t("runGovernance.createdBy")}</dt><dd><CopyableID value={selectedUnit.created_by_key_id} label={t("runGovernance.createdBy")} /></dd></div></dl></section>
        <section><div className="governance-detail-heading"><h3>{t("runGovernance.outcomeTimeline")}</h3><span>{selectedOutcomes.length}</span></div>{selectedOutcomes.length === 0 ? <p className="governance-muted">{t("runGovernance.noOutcomesDescription")}</p> : <ol className="governance-timeline">{selectedOutcomes.map((item) => <li key={item.id}><OutcomeBadge outcome={item} /><strong>{item.value}</strong><span>{item.definition_id} · v{item.definition_version}</span><span>#{item.revision} · {dateTime(item.observed_at, "full")}</span><CopyableID value={item.id} label={t("runGovernance.outcome")} /></li>)}</ol>}</section>
        <section><div className="governance-detail-heading"><h3>{t("runGovernance.runs")}</h3><span>{selectedUnitRuns.length}</span></div>{selectedUnitRuns.length === 0 ? <p className="governance-muted">{t("runGovernance.noRunsDescription")}</p> : <div className="governance-run-list">{selectedUnitRuns.map((run) => <article key={run.id} className={selectedRunID === run.id ? "selected" : ""}><span><CopyableID value={run.id} label={t("runGovernance.run")} /><RunStatusBadge status={run.status} /></span><span><BudgetStatusBadge status={run.budget_state} />{money(run.committed_micros_usd)}</span><button type="button" className="button ghost" aria-pressed={selectedRunID === run.id} onClick={() => onSelectRun(run.id)}>{t("runGovernance.viewAttempts")}</button></article>)}</div>}</section>
        {selectedRun && <section className="governance-run-detail"><div className="governance-detail-heading"><h3>{t("runGovernance.runDetails")}</h3><BudgetStatusBadge status={selectedRun.budget_state} /></div><RunBudgetBar committed={selectedRun.committed_micros_usd} reserved={selectedRun.reserved_micros_usd} budget={selectedRun.budget_micros_usd} /><dl className="governance-detail-list"><div><dt>{t("runGovernance.remaining")}</dt><dd>{money(selectedRun.remaining_micros_usd)}</dd></div><div><dt>{t("runGovernance.expiresAt")}</dt><dd>{dateTime(selectedRun.expires_at, "full")}</dd></div><div><dt>{t("runGovernance.closeReason")}</dt><dd>{selectedRun.close_reason || "—"}</dd></div></dl><h4>{t("runGovernance.attempts")}</h4>{attemptsPending && <Loading />}{Boolean(attemptsError) && <ErrorState error={attemptsError} />}{attempts && attempts.length === 0 && <p className="governance-muted">{t("runGovernance.noAttemptsDescription")}</p>}{attempts && attempts.length > 0 && <div className="governance-attempts">{attempts.map((attempt) => <article key={attempt.event_id}><div><GovernanceBadge tone={attempt.status === "success" ? "good" : "danger"}>{attempt.status === "success" ? t("runGovernance.attemptSuccess") : t("runGovernance.attemptFailed")}</GovernanceBadge><CopyableID value={attempt.request_id} label={t("runGovernance.request")} /></div><strong>{attempt.requested_model || "—"} <span aria-hidden="true">→</span> {attempt.provider_model || "—"}</strong><dl><div><dt>{t("runGovernance.tokens")}</dt><dd>{attempt.provider_input_tokens + attempt.provider_output_tokens}</dd></div><div><dt>{t("runGovernance.costEvidence")}</dt><dd><CostEvidence attempt={attempt} /></dd></div><div><dt>{t("runGovernance.latency")}</dt><dd>{attempt.latency_millis == null ? "—" : `${attempt.latency_millis} ms`}</dd></div><div><dt>{t("runGovernance.completedAt")}</dt><dd>{dateTime(attempt.completed_at, "full")}</dd></div></dl></article>)}</div>}</section>}
