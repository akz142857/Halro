...
export function OutcomeWorkspace({
  ...
  return <section className="governance-workspace" id="run-governance-panel" role="tabpanel" aria-labelledby="run-governance-tab-results">
    ...
    {view === "analytics" && <div id="run-governance-results-panel" role="tabpanel" aria-labelledby="run-governance-results-tab-analytics">
      ...
      {summary && <>
        <div className={`governance-completeness ${overallState}`} role="status"><div><GovernanceBadge tone={overallState === "complete" ? "good" : overallState === "unknown" ? "unknown" : "warning"}>{t(`runGovernance.summaryState.${overallState}`)}</GovernanceBadge><strong>{t(`runGovernance.summaryReason.${summary.outcome_reason || (outcomeState === "complete" ? "complete" : "missing_outcomes")}`)}</strong></div>{summary.unknown_attempts > 0 && <p>{t("runGovernance.unknownCostExplanation", { count: summary.unknown_attempts })}</p>}{summary.in_progress_cost_micros_usd > 0 && <p>{t("runGovernance.inProgressExplanation", { amount: money(summary.in_progress_cost_micros_usd) })}</p>}</div>
