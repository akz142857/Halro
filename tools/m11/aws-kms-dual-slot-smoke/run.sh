...
: "${HEIMDALL_AWS_KMS_PRIMARY_KEY_ARN:?set an existing customer-managed Primary KMS Key ARN}"
: "${HEIMDALL_AWS_KMS_RECOVERY_KEY_ARN:?set a different existing customer-managed Recovery KMS Key ARN}"

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
SOURCE_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RESULT_PATH=${1:-"$SOURCE_ROOT/.tmp/m11-aws-kms-dual-slot-smoke.json"}
TEST_OUTPUT=$(mktemp "${TMPDIR:-/tmp}/heimdall-aws-kms-dual-test.XXXXXX")
