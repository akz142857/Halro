...
func (s *Store) Put(record Record) (bool, error) {
	...
	if strings.ContainsAny(record.RequestID, `/\.`) {
		// The request ID becomes a filename. Halro's own IDs cannot contain
		// these, and a caller-supplied one reaches here through requestmeta —
		// so this is the line that keeps a supplied "../../master.key" from
	...
	path := filepath.Join(directory, record.RequestID+fileExtension)
...
func (s *Store) Get(requestID, projectID string) (Record, bool, error) {
	...
	for _, day := range days {
		sealed, err := os.ReadFile(filepath.Join(s.root, day, requestID+fileExtension))
...
// Purge removes every day older than the retention window. Whole directories,
// not individual records: retention is the promise, and a partial sweep that
// leaves a record behind breaks it in the direction that matters.
func (s *Store) Purge() error {
	...
	for _, day := range days {
		...
		if err != nil {
			// Not a day directory. Left alone rather than removed: this store
			// deletes on a clock, and deleting something it does not recognise
			// is how a retention sweep becomes a data loss incident.
			continue
		}
		// A day directory expires once its last possible instant is past the
		// cutoff, so a record written at 23:59 still gets its full window.
		if parsed.AddDate(0, 0, 1).After(cutoff) {
			continue
		}
		if err := os.RemoveAll(filepath.Join(s.root, day)); err != nil {
