...
func TestGovernanceReplicationSourceReadsExactFramesAndTruncatesTail(t *testing.T) {
	...
	for _, id := range []string{"one", "two"} {
		if _, err := log.Append(context.Background(), replicationHookEvent(id)); err != nil {
			t.Fatal(err)
		}
	}
