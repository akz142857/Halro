...
func BuiltinEndpointManifests() []EndpointCompatibilityManifest {
	...
	chatCoverage := []ProfileCoverage{
		...
		{ProfileID: domain.ProfileBedrockMantleResponses, UnsupportedRequestFields: []string{"n", "stop", "seed", "reasoning_effort"}, DeclaredTransforms: []string{"Chat messages are mapped to stateless Responses input items", "store=false is always sent upstream", "streaming requests with tools are rejected before provider I/O"}},
	...
	responseCoverage := []ProfileCoverage{
		...
		{ProfileID: domain.ProfileBedrockMantleResponses, DeclaredTransforms: []string{"stateless Responses are sent directly with store=false"}},
