...
func BuiltinEndpointManifests() []EndpointCompatibilityManifest {
	...
	manifests := []EndpointCompatibilityManifest{
		...
		{ID: "openai.responses.stateless.v1", NorthboundProfile: ProfileOpenAIResponses, ProfileRevision: 1, Protocol: "openai", Method: "POST", Path: "/v1/responses", SemanticOperation: semantic.OperationGenerate,
			RequestFields:         []string{"model", "input", "input[].type", "input[].role", "input[].content", "input[].call_id", "input[].name", "input[].arguments", "input[].output", "input[].content[].type", "input[].content[].text", "input[].content[].detail", "instructions", "reasoning", "stream", "store", "temperature", "top_p", "max_output_tokens", "tools", "tools[].type", "tools[].name", "tools[].description", "tools[].parameters", "tools[].strict", "tool_choice", "parallel_tool_calls", "text.format", "text.format.type", "text.format.name", "text.format.description", "text.format.schema", "text.format.strict", "user"},
