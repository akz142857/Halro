...
func BuiltinEndpointManifests() []EndpointCompatibilityManifest {
	...
	manifests := []EndpointCompatibilityManifest{
		...
		{ID: "anthropic.messages.2023-06-01", NorthboundProfile: ProfileAnthropicMessages, ProfileRevision: 1, Protocol: "anthropic", Method: "POST", Path: "/v1/messages", SemanticOperation: semantic.OperationGenerate,
			RequestFields: []string{"model", "max_tokens", "messages", "messages[].role", "messages[].content", "messages[].content[].type=document", "messages[].content[].type=search_result", "system", "stream", "stop_sequences", "temperature", "top_p", "top_k", "tools", "tools[].type=custom", "tools[].type=bash_*", "tools[].type=text_editor_*", "tools[].type=memory_*", "tools[].type=computer_*", "tool_choice", "thinking", "metadata", "service_tier", "output_config", "output_config.effort", "output_config.format"}, RejectedRequestFields: []string{"provider-executed tools (web_search_*, web_fetch_*, code_execution_*, advisor_*, tool_search_*)", "mcp_servers", "container", "fallbacks", "Anthropic-defined tools in portable mode", "strict tools in portable mode", "signed thinking in portable mode", "unknown fields"}, RequestHeaders: []string{"x-api-key or Authorization", "anthropic-version", "anthropic-beta", "Halro-Route-Mode", "Content-Type"}, ResponseFields: []string{"id", "type", "role", "content", "model", "stop_reason", "stop_sequence", "usage"}, StreamEvents: []string{"message_start", "content_block_start", "content_block_delta", "content_block_stop", "message_delta", "message_stop", "ping", "error"}, StateSemantics: "stateless; portable is default; native pins one exact Anthropic-wire provider profile and disables cross-provider fallback", SDKMatrix: []string{"anthropic-go", "anthropic-typescript", "anthropic-python"}, Status: StatusCompatible, DocumentedDeviations: []string{"only anthropic-version 2023-06-01 is accepted", "anthropic-beta is forwarded only in native mode and only for tokens the selected connection has been configured to accept; portable mode rejects it because the request is re-authored through the canonical model and a beta token describes the request as written", "tools are classified by execution site: Anthropic-defined client-executed tools are accepted at any version suffix, while provider-executed tools are rejected because the upstream would make network calls outside SafeTransport's host allowlist", "mcp_servers, container, and fallbacks are rejected as deliberate boundaries rather than unmodelled fields: the first two delegate egress or code execution to the upstream, and the third moves model selection outside Halro's routing and cost attribution", "tool and content-block bodies are forwarded verbatim, so cache_control and per-tool configuration reach the provider unchanged", "Gateway Keys are accepted through x-api-key for official SDK compatibility and are never forwarded upstream", "native mode is selected with Halro-Route-Mode and requires either the direct Anthropic or Bedrock Mantle Anthropic profile"}, ProviderProfiles: chatProfiles, ProfileCoverage: []ProfileCoverage{
