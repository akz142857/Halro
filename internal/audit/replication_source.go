...
// ReplicationCursor authenticates the file and returns its native generation
// and sequence. Audit has one generation for the lifetime of an incarnation.
...
func ReplicationHeadAt(path string, key []byte, generation, sequence uint64) ([32]byte, error) {
	if generation != 1 {
		return [32]byte{}, errors.New("audit replication generation must be 1")
	}
	if sequence == 0 {
		return [32]byte{}, nil
	}
	...
	if sequence > summary.Records || head == ([32]byte{}) {
		return [32]byte{}, errors.New("audit replication baseline is outside the durable log")
	}
	return head, nil
}

// ReadReplicationFrames returns the exact authenticated frame bytes for an
// inclusive sequence range.
func ReadReplicationFrames(path string, key []byte, first, last uint64) ([]byte, error) {
	if first == 0 || last < first {
		return nil, errors.New("audit replication range is invalid")
	}
	file, err := os.Open(path)
	if err != nil {
		return nil, err
	...
	if start < 0 || end < start {
		return nil, errors.New("audit replication range is outside the durable log")
	}
...
// TruncateReplicationTail removes only a verified suffix and fsyncs the file.
func TruncateReplicationTail(path string, key []byte, sequence uint64) error {
	...
	if sequence > summary.Records || sequence > 0 && offset == 0 {
		return errors.New("audit truncation cursor is outside the durable log")
	}
